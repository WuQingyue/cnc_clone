<template>
    ...
    >
      ...
      <el-table-column label="模型信息" min-width="150" align="center">
        <template #default="scope">
          <el-button link type="primary" @click="openModelInfoDialog(scope.row)">
            查看详情
          </el-button>
        </template>
      </el-table-column>
      <el-table-column label="加工金额" min-width="150" align="center">
        <template #default="scope">
          <el-button link type="primary" @click="openAmountDialog(scope.row)">
            查看详情
          </el-button>
        </template>
      </el-table-column>
      <el-table-column prop="shipping_cost" label="物流信息" min-width="150" align="center">
        <template #default="scope">
          <el-button link type="primary" @click="openShippingDialog(scope.row.order_no, scope.row.shipping_cost)">
            查看详情
          </el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" fixed="right" min-width="280" align="center">
        <template #default="scope">
          <el-select v-model="scope.row.status" placeholder="选择操作" @change="handleStatusChange(scope.row)">
            <el-option label="待加工" value="pending_processing" />
            <el-option label="加工中" value="processing" />
            <el-option label="加工完成-未揽货" value="completed_not_collected" />
            <el-option label="已揽货" value="collected" />
...
import axios from 'axios'
  ...
  const handleStatusChange = async (row) => {
    ...
    if (row.status == 'completed_not_collected') {
      try {
        const response = await service.post(`/api/logistics/order_create/${row.order_no}`, { withCredentials: true });
